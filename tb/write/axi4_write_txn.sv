...
// axi4_write_txn.sv
//
// Randomizable write-burst transaction.
//
// The coverage plan in axi4_write_env.sv deliberately directs a small
// set of initial randomized transactions into every meaningful
// functional-coverage bin. After that plan is complete, normal
// constrained-random stimulus continues for the remaining bursts.
//=============================================================
...
class axi4_write_txn;
    ...
    constraint c_awaddr {
        (addr_mode == ADDR_NORMAL)
            -> awaddr inside {[16'h0000:16'h0F00]};

        (addr_mode == ADDR_NEAR_BOUNDARY)
            -> awaddr inside {[16'h0FA0:16'h0FFC]};

        (addr_mode == ADDR_OUT_OF_RANGE)
            -> awaddr inside {[16'h1000:16'hFFFF]};
    ...
    function string convert2string();
        return $sformatf(
            "AWADDR=0x%0h AWLEN=%0d AWSIZE=%0d mode=%s",
            awaddr, awlen, awsize, addr_mode.name()
        );
    endfunction
