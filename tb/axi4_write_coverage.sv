...
//
// Functional coverage for the write-only verification environment.
// Coverage is sampled from the stimulus/golden-model transaction,
// not from the DUT result, so functional coverage measures which
// scenarios were exercised while the scoreboard measures correctness.
//=============================================================
...
class axi4_write_coverage;
    ...
    covergroup cg_write;
        ...
        cx_size_bresp: cross cp_awsize, cp_bresp {
            ...
            ignore_bins subword_okay =
                binsof(cp_awsize.sub_word_size) && binsof(cp_bresp.okay);

            ignore_bins oversize_okay =
                binsof(cp_awsize.oversize) && binsof(cp_bresp.okay);
        }
    ...
    covergroup cg_beat;
        ...
        cp_beat_valid: coverpoint beat_valid_bit {
            bins invalid = {0};
            bins valid   = {1};
        }

        cp_beat_position: coverpoint {beat_is_first, beat_is_last} {
